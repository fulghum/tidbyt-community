"""
Applet: GitHub
Summary: Display GitHub stargazer count
Description: Display stargazer count for a GitHub repo.
Author: fulghum
"""

load("encoding/base64.star", "base64")
load("encoding/json.star", "json")
load("http.star", "http")
load("humanize.star", "humanize")
load("render.star", "render")
load("schema.star", "schema")

GITHUB_REPO_SEARCH_URL  = "https://api.github.com/search/repositories?q=%s"
GITHUB_ISSUE_SEARCH_URL = "https://api.github.com/search/issues?q=%s"

GITHUB_ICON = base64.decode("""

""")

def send_github_request(url, query_params):
    res = http.get(
        url = url % humanize.url_encode(query_params),
    )
    if res.status_code != 200:
        # buildifier: disable=print
        print("GitHub API request failed: %s - %s " % (res.status_code, res.body()))
        return None

    return json.decode(res.body())

def print_bug_count():
    org_name = "dolthub"
    repo_name = "dolt"
    query_params = "repo:%s/%s is:issue label:bug is:open" % (org_name, repo_name)
    bug_count_url = GITHUB_ISSUE_SEARCH_URL % humanize.url_encode(query_params)
    print("GitHub issue search url: %s" % bug_count_url)
    res_json = send_github_request(GITHUB_ISSUE_SEARCH_URL, query_params)
    bug_count = res_json["total_count"]
    print("bug_count: %s" % bug_count)

def main(config):    
    # TODO: Read from app config
    org_name = 'dolthub'
    repo_name = 'dolt'

    print("Fetching GitHub stargazer count...")
    # TODO: Add caching
    query_params = "repo:%s/%s" % (org_name, repo_name)
    res_json = send_github_request(GITHUB_REPO_SEARCH_URL, query_params)

    stargazer_count = res_json["items"][0]["stargazers_count"]
    print("stargazer_count: %s " % stargazer_count)


    image_size = 16
    msg = "%s stars" % humanize.comma(stargazer_count)

    display_name = "%s/%s" % (org_name, repo_name)
    username_child = render.Text(
        color = "#00FF00",
        content = display_name,
    )

    if len(display_name) > 12:
        username_child = render.Marquee(
            width = 64,
            child = username_child,
        )

    # https://docs.github.com/en/search-github/searching-on-github/searching-issues-and-pull-requests
    print_bug_count()

    return render.Root(
        child = render.Box(
            render.Column(
                expanded = True,
                main_align = "space_evenly",
                cross_align = "center",
                children = [
                    render.Row(
                        expanded = True,
                        main_align = "space_evenly" if len(msg) > 5 else "center",
                        cross_align = "center",
                        children = [
                            render.Padding(
                                pad = (1, 1, 1, 1),
                                child = render.Image(GITHUB_ICON, height = image_size, width = image_size),
                            ),
                            render.WrappedText(msg, font = "tb-8" if len(msg) > 7 else "6x13"),
                        ],
                    ),
                    username_child,
                ],
            ),
        ),
    )

def get_schema():
    return schema.Schema(
        version = "1",
        fields = [],
    )